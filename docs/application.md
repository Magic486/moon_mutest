...
`moon_mutest` 的重点不是做一个简单的源码替换脚本，而是围绕 MoonBit 工具链做一套
可复用、可执行、可集成的变异测试基础设施。它的创新点和特点主要体现在以下方面：

1. 面向 MoonBit 生态的原生变异测试工具

   目前 MoonBit 生态中常见质量手段主要集中在 `moon check`、`moon test` 和常规测试
   覆盖。本项目把 mutation testing 引入 MoonBit 工程实践，补充“测试是否真的能发现
   缺陷”这一维度，能直接服务 MoonBit 开源库、工具链项目和应用项目的质量提升。

2. 从源码扫描到真实执行的完整闭环

   项目不只输出 mutation 候选点，而是进一步支持复制真实 workspace、执行 baseline、
   逐个应用 mutant、运行 `moon check` / `moon test`、恢复文件并汇总结果。这个闭环让
   工具具备实际使用价值，而不是停留在静态分析或演示层面。

3. 库 API 与 CLI 双形态设计

   核心能力以 MoonBit 库形式组织，便于其他 MoonBit 工具复用扫描、规划、报告和质量
   门禁能力；同时提供 CLI，方便开发者在本地和 CI 中直接使用。这个设计兼顾了生态复用
   和落地体验。

4. 基线门禁和临时 workspace 机制

   runner 先在临时副本中执行 baseline，只有原项目本身通过检查和测试后才继续执行
   mutants。mutation 应用也发生在临时 workspace 内，避免污染用户原项目。这一点对
   实际工程使用非常关键。

5. 面向 CI 的选择、分片和质量门禁能力

   变异测试天然成本较高，本项目从一开始就设计了 `--max-mutants`、`--first`、id 范围、
   分片、批次规划和质量门禁，方便后续接入 CI、nightly job 或大型项目的渐进式检查。

6. 保守扫描与可解释报告

   扫描器会跳过字符串、字符字面量、行注释和块注释，减少明显误报。每个 mutant 都保留
   文件、位置、规则标签和 patch preview，报告可输出为文本、Markdown 或 JSON，便于
   人读、机器读和后续工具集成。

这些特点使 `moon_mutest` 不只是“能跑”的比赛项目，而是一个可以继续成长为 MoonBit
测试质量基础设施的开源工具。
...
- 生态价值明确：直接补充 MoonBit 测试质量评估工具，而不是重复已有包；
- 技术路径完整：覆盖扫描、规划、执行、恢复、报告、评分和门禁；
- 工程边界清晰：核心库保持可复用，真实 runner 独立承担平台相关能力；
- 使用体验完整：既能快速扫描代码片段，也能对真实项目执行自动化 mutation run；
- 后续扩展自然：可以继续增加语法级规则、增量运行、CI 报告和 mooncakes.io 发布；
- 展示效果直观：通过 survived mutants 能直接指出测试薄弱区域，便于评审和用户理解。

与一般代码覆盖率工具相比，本项目关注的是“测试能否杀死被注入的缺陷”；与简单脚本式
mutation 工具相比，本项目更重视 MoonBit 项目结构、临时 workspace、安全恢复、质量门禁
和可复用 API。这些设计让它更适合作为 MoonBit 生态基础设施长期维护。
...
创新点与特点：项目不是简单包装 `moon test`，而是提供面向 MoonBit 的原生变异测试闭环：
源码扫描、mutation 规划、临时 workspace 复制、baseline 检查、真实命令执行、结果分类、
报告输出和质量门禁。它同时提供库 API 与 CLI，既能被其他 MoonBit 工具复用，也能直接
接入本地开发和 CI。
